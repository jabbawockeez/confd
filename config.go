...
type Config struct {
	...
	ConfigFile    string
	OneTime       bool
	ResultKey     string `toml:"result-key"`
}
...
func init() {
	...
	flag.StringVar(&config.ResultKey, "result-key", "/confdResult", "key to store check_cmd and reload_cmd result (only used with -backend=etcdv3)")
}
